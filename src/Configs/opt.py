...
class ThisOPTAttention(OPTAttention):
    ...
    def __init__(
        ...
        if is_cross_attention:
            ...
            #self.k_proj = nn.Linear(config.encoder_hidden_size, int(embed_dim / self.cross_attention_reduce_factor), bias=bias)
            self.k_proj = nn.Linear(768, int(embed_dim / self.cross_attention_reduce_factor), bias=bias)
            #print("self.k_proj",self.k_proj)
            #self.v_proj = nn.Linear(config.encoder_hidden_size, int(embed_dim / self.cross_attention_reduce_factor), bias=bias)
