...
class ThisGPT2Attention(GPT2Attention):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.is_cross_attention:
            #self.c_attn = Conv1D(int(2 / self.cross_attention_reduce_factor * self.embed_dim), config.encoder_hidden_size) 
            self.c_attn = Conv1D(int(2 / self.cross_attention_reduce_factor * self.embed_dim), 768) 
