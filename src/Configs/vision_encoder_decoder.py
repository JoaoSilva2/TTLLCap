...
class ArcSin():
    def __init__(self, s=0.55, Np=100) -> None:
        self.alpha_r = [0.0, np.pi / 2]
        self.s = s
        self.Np = Np
        self.w = 0.04
    ...
    def forward(self, features):
        ...
        #return torch.nn.functional.normalize(features, dim=2)

        features = torch.nn.functional.normalize(features, dim=2)
        scalars = torch.rand(features.shape[0], device=features.device).unsqueeze(-1).unsqueeze(-1)
        features += 1.0 * scalars * torch.randn(features.shape, device=features.device)
        return torch.nn.functional.normalize(features, dim=2)
        
        """
        features = torch.clamp(features, min=-1.0, max=1.0)
        alpha = torch.FloatTensor(1, 1).uniform_(self.alpha_r[0], self.alpha_r[1]).item()

        if torch.all(torch.asin(features) + alpha < np.pi / 2):
            delta_p = torch.sin(torch.asin(features) + alpha) - features
        else:
            delta_p = 1 - features
        
        if torch.all(torch.asin(features) - alpha > -np.pi / 2):
            delta_m = torch.sin(torch.asin(features) - alpha) - features
        else:
            delta_m = -1 - features
        

        noise_pool = torch.randn(features.shape[0], self.Np, features.shape[2]).to("cuda")
    
        if(torch.all(noise_pool > 0)):
            augmented_features = features + noise_pool * delta_p
        else:
            augmented_features = features + noise_pool * (-delta_m)

        aux = torch.nn.functional.normalize(augmented_features, dim=2)
        similarities = torch.nn.functional.cosine_similarity(features, aux, dim=2)
        
        idxs = torch.argmax(similarities, dim=1)

        selected_features = []
        max_similarities = []
        for i in range(len(similarities)):
            idx = idxs[i]
            selected_features.append([noise_pool[i][idx].tolist()])
            max_similarities.append(similarities[i][idx].tolist())


        selected_features = torch.Tensor(selected_features).to("cuda")
        max_similarities = torch.Tensor(max_similarities).to("cuda")
        

        if torch.mean(max_similarities) < self.s - 0.01:
            self.alpha_r[1] = alpha
        elif torch.mean(max_similarities) > self.s + 0.01:
            self.alpha_r[0] = alpha
        """
        return torch.nn.functional.normalize(selected_features, dim=2)
...
class SmallCap(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.training:
            encoder_hidden_states = self.noise.forward(encoder_hidden_states)
